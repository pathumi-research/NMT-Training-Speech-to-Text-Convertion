...
FILE_NAME = "merged_f.txt"
MODEL_BASE_DIR_NAME = "m2m100_base_saved"
OUTPUT_DIR_NAME = "m2m100_finetuned"

FILE_PATH = os.path.join(PROJECT_DIR, FILE_NAME)
OUTPUT_DIR = os.path.join(PROJECT_DIR, OUTPUT_DIR_NAME)
model_save_path = os.path.join(PROJECT_DIR, MODEL_BASE_DIR_NAME)
...
if os.path.exists(model_save_path) and len(os.listdir(model_save_path)) > 0:
    print(f"📂 Found saved model. Loading from: {model_save_path}")
...
tokenizer.src_lang = "si"
tokenizer.tgt_lang = "si"
print(f"Tokenizer set for src_lang: {tokenizer.src_lang}, tgt_lang: {tokenizer.tgt_lang}")

# ====================================================================
# STEP 3: VOCAB ADAPTATION
# ====================================================================
print("\n3. Adapting vocabulary for Gloss tokens...")
unique_gloss_tokens = set()
for text in train_tgt + eval_tgt:
    parts = text.split('|')
    for part in parts:
        if ':' in part:
            gloss_token = part.split(':', 1)[1]
...
num_added_toks = tokenizer.add_tokens(unique_gloss_tokens_list)
model.resize_token_embeddings(len(tokenizer))
print(f"Added {num_added_toks} new tokens. Model vocab size: {len(tokenizer)}")
...
def preprocess_function(examples):
    model_inputs = tokenizer(examples["source"], max_length=MAX_SEQ_LENGTH, truncation=True)
    with tokenizer.as_target_tokenizer():
        labels = tokenizer(examples["target"], max_length=MAX_SEQ_LENGTH, truncation=True)
    model_inputs["labels"] = labels["input_ids"]
    return model_inputs

print("\n4. Tokenizing datasets...")
tokenized_train = train_dataset.map(preprocess_function, batched=True).remove_columns(["source", "target"])
tokenized_eval = eval_dataset.map(preprocess_function, batched=True).remove_columns(["source", "target"])
print("Tokenization complete.")
...
# Save final model/tokenizer safely
trainer.save_model(OUTPUT_DIR)
tokenizer.save_pretrained(OUTPUT_DIR)
print(f"Final model and tokenizer saved to: {OUTPUT_DIR}")
